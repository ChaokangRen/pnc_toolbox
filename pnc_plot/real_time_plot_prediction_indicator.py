...
ax_demo = fig.add_subplot(111, aspect=1)
fig.tight_layout()
...
def callback_debuginfo(debuginfo_msg):
    ...
    if(has_indicator_msg):
        indicator_info = info.split("indicator:")[1].split("indicator_end")[0]
        ...
        if(count_indi % 10 == 0):
            ...
            for i8 in range(count_indicator):
                ...
                if (predictor == 0):
                    if(obs_error <= 1):
                        indicator0_green_x.append(obs_his_x)
                        indicator0_green_y.append(obs_his_y)
                    if(obs_error > 1 and obs_error <= 5):
                        indicator0_yellow_x.append(obs_his_x)
                        indicator0_yellow_y.append(obs_his_y)
                    if(obs_error > 5):
                        indicator0_red_x.append(obs_his_x)
                        indicator0_red_y.append(obs_his_y)
                if (predictor == 1):
                    if(obs_error <= 1):
                        indicator1_green_x.append(obs_his_x)
                        indicator1_green_y.append(obs_his_y)
                    if(obs_error > 1 and obs_error <= 5):
                        indicator1_yellow_x.append(obs_his_x)
                        indicator1_yellow_y.append(obs_his_y)
                    if(obs_error > 5):
                        indicator1_red_x.append(obs_his_x)
                        indicator1_red_y.append(obs_his_y)


def data_gen(t=0.0):
    while True:

        yield t, lane_x_list_list, lane_y_list_list, obs_x_list_list, obs_y_list_list, \
            point_x_list_list, point_y_list_list, indicator0_green_x, indicator0_green_y, \
            indicator0_yellow_x, indicator0_yellow_y, indicator0_red_x, indicator0_red_y,\
            indicator1_green_x, indicator1_green_y, indicator1_yellow_x, indicator1_yellow_y, \
            indicator1_red_x, indicator1_red_y
...
def run(data):
    time, lane_xs, lane_ys, obs_xs, obs_ys, traj_xs, traj_ys, g_x0, g_y0, \
        y_x0, y_y0, r_x0, r_y0, g_x1, g_y1, y_x1, y_y1, r_x1, r_y1 = data
    ...
    #         print("errori7")
    try:
        line_0 = ax_demo.scatter(g_x0, g_y0, s=20,  marker='^',
                                 color='green')
    except:
        pass
    try:
        ax_demo.scatter(y_x0, y_y0, s=20,  marker='^',
                        color='orange')
    except:
        pass
    try:
        ax_demo.scatter(r_x0, r_y0, s=20, marker='^',
                        color='red')
    except:
        pass

    try:
        line_1 = ax_demo.scatter(g_x1, g_y1, s=20, marker='x',
                                 color='green')
    except:
        pass
    try:
        ax_demo.scatter(y_x1, y_y1, s=20, marker='x',
                        color='orange')
    except:
        pass
    try:
        ax_demo.scatter(r_x1, r_y1, s=20, marker='x',
                        color='red')
    except:
        pass
    ...
    try:
        ax_demo.legend((line_0, line_1), (u'lane_sequence_predictor',
                                          u'free_move_predictor'), loc=1)
    except:
        pass
